    ...
    render() {
        return (
            <SafeAreaView>
                ...
                />
                <button onClick={saveReviewToLocalStorage()}>Save Review</button>
            </SafeAreaView>
        );
    }
}
...
function saveReviewToLocalStorage(gameID, reviewText) {
    //saves in gameReviews storage
    const STORAGE_KEY = '@gameReviews';

    //generates a 16 digit id for the review
    const reviewID = generateID(16);

    const saveReview = async () => {
        try {
            await AsyncStorage.setItem(STORAGE_KEY, reviewID, gameID, reviewText)
            alert('Data saved');
        } catch (e) {
